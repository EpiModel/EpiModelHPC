...
- "data/run/": raw files produced and used by the code. They are not tracked by `git`.
...
hpc_configs <- list(
  ...
  r_loader = "module load R/4.2.1",
  r_version = "4.2.1"
)
```

The `hpc_configs` list holds the Slurm configuration for your cluster. Adapt
the partition, R version, and mail settings to your environment.
...
*notes*: SLURM configuration can vary, on [HYAK](https://hyak.uw.edu/) for instance
  ...
  option. Adapt the `hpc_configs` list to match your cluster's requirements.
