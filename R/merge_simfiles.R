...
#' @title Extract Epidemiological Output from Saved Simulation Files and Resave.
#'
#' @description Loads a series of simulation files processed with \code{process_simfiles},
#'              extracts epidemiological output for smaller file sizes, and then
#'              saves the output.
#'
#' @inheritParams process_simfiles
#' @param suffix Suffix to add to file name before saving the extracted files.
#'
#' @export
#'
get_epi <- function(simno = NA, indir = "data/", outdir = "data/",
                    vars = NULL, suffix = "lim", compress = "xz") {

  if (is.na(simno)) {
    fn <- list.files(indir, pattern = "sim.*.rda", full.names = TRUE)
  } else {
    fn <- list.files(indir, pattern = paste0("sim.n", simno, ".rda"), full.names = TRUE)
  }
  ...
  for (i in seq_along(fn)) {
    ...
    save(sim, file = paste0(outdir, "/sim.n", nums[i], ".", suffix, ".rda"), compress = compress)
  }
